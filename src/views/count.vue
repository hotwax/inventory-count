    ...
    <ion-content>
      ...
      <div class="product-info">
        ...
        <div class="product-features">
          <ion-item lines="none">
            <ion-chip
              v-if="$filters.getFeature(product.featureHierarchy, '1/COLOR/')"
            >
              <ion-icon :icon="colorPaletteOutline" />
              <ion-label>{{
                $filters.getFeature(product.featureHierarchy, "1/COLOR/")
              }}</ion-label>
            </ion-chip>
            <ion-chip
              v-if="$filters.getFeature(product.featureHierarchy, '1/SIZE/')"
            >
              <ion-icon :icon="resize" />
              <ion-label>{{
                $filters.getFeature(product.featureHierarchy, "1/SIZE/")
              }}</ion-label>
            </ion-chip>
          </ion-item>
        </div>
      ...
      <div
        class="segment"
        v-if="
          hasPermission(Actions.APP_INVNTRY_CNT_IMPORT) &&
          hasPermission(Actions.APP_VARIANCE_LOG)
        "
      >
        <ion-segment v-model="segmentSelected">
          <ion-segment-button
            v-if="hasPermission(Actions.APP_INVNTRY_CNT_IMPORT)"
            value="count"
          >
            <ion-label>{{ $t("Count") }}</ion-label>
          </ion-segment-button>
          <ion-segment-button
            v-if="hasPermission(Actions.APP_VARIANCE_LOG)"
            value="variance"
          >
      ...
      <div v-if="segmentSelected === 'count'" class="inventory-form">
        <ion-item>
          ...
          <ion-input
            ...
            inputmode="numeric"
            :value="quantity"
            @ionChange="quantity = $event.detail.value"
          ></ion-input>
        </ion-item>
        <ion-item lines="none">
          <ion-note id="stockCount">{{
            $t("Enter the count of stock on the shelf.")
          }}</ion-note>
        </ion-item>
        <ion-item>
        ...
        <ion-item v-if="viewQOH">
          <ion-label>{{ $t("Variance") }}</ion-label>
          <ion-label slot="end">{{
            availableQOH && quantity ? quantity - availableQOH : "-"
          }}</ion-label>
        </ion-item>
        ...
        <div class="action">
          <ion-button
            size="large"
            :disabled="!hasPermission(Actions.APP_INVNTRY_CNT_IMPORT)"
            @click="updateProductInventoryCount()"
          >
      ...
      <div v-else class="inventory-form">
        <ion-item>
          ...
          <ion-input
            type="number"
            inputmode="numeric"
            :placeholder="$t('inventory variance')"
            v-model="product.varianceQuantity"
          />
        </ion-item>
        <ion-item lines="none">
          <ion-note id="stockCount">{{
            $t("Enter the amount of stock that has changed.")
          }}</ion-note>
        </ion-item>
        ...
        <ion-item>
          ...
          <ion-select
            interface="popover"
            :value="product.varianceReasonId"
            @ionChange="updateVarianceReason($event)"
            :placeholder="$t('Select reason')"
          >
            <ion-select-option
              v-for="reason in varianceReasons"
              :key="reason.enumId"
              :value="reason.enumId"
...
export default defineComponent({
  ...
  methods: {
    async selectLocation() {
      const pickerOptions = await this.facilityLocations.map(
        (location: any) => {
    ...
    updateProductInventoryCount() {
      if (this.quantity >= 0) {
        ...
        this.store.dispatch("product/updateInventoryCount", {
          ...this.product,
        ...
        showToast(translate("The stock count should be positive"));
      } else {
    ...
    async getInventory() {
      if (!this.product.locationId) {
        console.warn(
          "There are no facility locations, skipping fetching the inventory"
        );
      }
      try {
    ...
    async confirmVarianceUpdate() {
      if (this.product.varianceQuantity && this.product.varianceReasonId) {
        const alert = await alertController.create({
          ...
          message: this.$t(
            "The inventory will be immediately updated and cannot be undone. Are you sure you want to update the inventory variance?"
          ),
          buttons: [
            {
              text: this.$t("Cancel"),
