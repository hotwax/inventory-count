...
This document serves as the central reference for all training, setup, and architectural decisions made for the `e2e` Playwright automation project.
...
STORE_USER_USERNAME=your_store_user_username_here
STORE_USER_PASSWORD=your_store_user_password_here
OMS_NAME=your-oms-name
CLIENT_UAT_URL=https://inventorycount-uat.hotwax.io/login
CLIENT_PROD_URL=https://inventorycount.hotwax.io/bulkUpload
```

### Direct Environment URLs (For Reference)
- **Client UAT (Login):** `https://inventorycount-uat.hotwax.io/login`
- **Client PROD (Bulk Upload):** `https://inventorycount.hotwax.io/bulkUpload`

## 3. Login Flow & Launchpad Navigation
- **Login Flow:** Navigate to `https://launchpad.hotwax.io/home` -> Click Login -> Enter OMS Name (`your-oms-name`) -> Enter SSO Credentials -> Redirect back to authenticated Launchpad.
...
Welcome to the automation project! Follow these steps to run the scripts locally:

### Step 1: Environment Setup
You must create a `.env` file in the root of the project directory. This file is ignored by Git to keep credentials secure.
```env
